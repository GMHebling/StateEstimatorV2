...
// Estado do ramo
typedef enum { aberto, fechado } ESTADO;
...
typedef struct {
  ...
  std::complex<double> Vref[3];
  std::complex<double>
      Vinicial[3]; // recebe de arquivo de entrada a tensão inicial

  DLOAD loads[10]; // cargas conectadas à barra - limite de 10 cargas por barra
                   // para não ter que trabalhar na alocação dinâmica dentro do
                   // array de struct
  long int nloads;
...
typedef struct {
  FASES fases;
  double comprimento;
  std::complex<double> Zaa, Zab, Zac, Zbb, Zbc, Zcc;
...
typedef struct {
  ...
  long int
      controle; // (0 = Somente Forward sem Restrição / 1 = Locked Forward / 2 =
                // Locked Reverse / 3 = Bidirectional / 4 = Idle / 5 = Neutral
                // Reverse / 6 = Cogenartion / Ver manual Siemens MJ4A)
  double tap[3];
  double deltaVr, R1r, X1r, R2r, X2r, R3r, X3r, V1r, V2r,
      V3r; // Parametros de controle reverso
...
typedef struct {
  ...
  std::complex<double> **Ypp;
  std::complex<double> **Yps;
  std::complex<double> **Ysp;
  std::complex<double> **Yss;
  double tap_pri[3];
  double tap_sec[3];
...
typedef struct {
  ...
  // Medidores
  long int nmed;
  DMED **medidores; /**< Lista dos medidores. Limite de 20 para evitar alocação
                       dinamica*/

  // Grandezas elétricas que caracterizam o ramo
  std::complex<double> S[3];
  std::complex<double> Cur[3];
...
typedef struct {
  ...
  long int
      numeroAdjacentes; /**< Indica o tamanho da lista de adjacentes do nó. */
  NOADJACENTE adjacentes[15]; /**< Lista dos nós adjacentes. Limite de 15 para
                                 evitar alocação dinamica*/
  ...
  std::complex<double> Ysh[3][3];

  // Grandezas elétricas que caracterizam a barra
  std::complex<double> S[3];
  std::complex<double> V[3];
  std::complex<double> Cur[3];
  std::complex<double> V_aux[3];
  double V_aux_Sig[3];
  std::complex<double> V_est[3];
  std::complex<double> V_coord[3];
  int estimado;
} GRAFO;
