...
public:
  ...
  void initialize() {
    ...
    std::cout << "Reading Vinicial file\n";
    ...
    std::cout << "Building network model\n";
    topology.buildNetworkModel(grafo, numeroBarras, allRamos, numeroRamos,
                               Sbase);
    std::cout << "Initialization Completed\n";
  }

  void processVirtualMeasurements() {
    std::cout << "Reading DVMED file for HATCHEL estimator\n";
    medidasVirtuais =
        leituraMedidas(allRamos, numeroRamos, barras, numeroBarras, grafo,
                       Sbase, numeroMedidasVirtuais, 1);
    std::cout << "Virtual Measurement preprocessing\n";
    medidasVirtuais_pu = processaMedidas(medidasVirtuais, numeroMedidasVirtuais,
                                         grafo, numeroBarras);
  }
...
private:
  ...
  void readConfigFile(std::string &dataDir, std::string &dataFolder) {
    ...
    dataFolder = pasta;
    fgets(linha, 1000, config);
    estType = getField(linha, 1);
    this->estimatorType = atoi(estType);
    fgets(linha, 1000, config);
    idSim = getField(linha,1);
    this->idSIM = atoi(idSim);
    fgets(linha, 1000, config);
  ...
  DBAR *leituraDBAR(int &numeroBarras, int &numeroAlimentadores) {
    ...
    char blocoLeitura[2000]; /* Variável para realizar a leitura do bloco de
                                caracteres do arquivo. */
    char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar
                    o ponteiro da string lida do arquivo de forma a realizar o
                    loop no sscanf. */
    int contador = 0, i, aux, k; /* Variáveis contadores para percorrer o
                                    arquivo e a string de leitura. */
    int carac,
        numLinhas =
            0; /* Variável com o número de linhas do arquivo a serem lidas. */
    ...
    while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
      ...
      if (aux == -1) { // Criando novo DBAR
        ...
        m_barras[contador].ligacao =
            static_cast<LIGACAO>(atoi(getField(dados, 2)));
        m_barras[contador].fases = static_cast<FASES>(atoi(getField(dados, 3)));
        m_barras[contador].Vbase = atof(getField(dados, 4)) / pow(3.0, 0.5);
        ...
        m_barras[contador].Vinicial[0] = std::complex<double>(
              1.0 * cos(0.0 * M_PI / 180), 1.0 * sin(0.0 * M_PI / 180));
          m_barras[contador].Vinicial[1] = std::complex<double>(
              1.0 * cos(-120.0 * M_PI / 180), 1.0 * sin(-120.0 * M_PI / 180));
          m_barras[contador].Vinicial[2] = std::complex<double>(
              1.0 * cos(120.0 * M_PI / 180), 1.0 * sin(120.0 * M_PI / 180));
        ...
        if (getField(dados, 13) != NULL) {
          ...
          m_barras[contador].Vref[0] = std::complex<double>(
              VA * cos(TA * M_PI / 180), VA * sin(TA * M_PI / 180));
          m_barras[contador].Vref[1] = std::complex<double>(
              VB * cos(TB * M_PI / 180), VB * sin(TB * M_PI / 180));
          m_barras[contador].Vref[2] = std::complex<double>(
              VC * cos(TC * M_PI / 180), VC * sin(TC * M_PI / 180));
        ...
        if ((PA != 0) || (PB != 0) || (PC != 0) || (QA != 0) || (QB != 0) ||
          ...
          m_barras[aux].loads[k].lig =
              static_cast<LIGACAO>(atoi(getField(dados, 2)));
          m_barras[aux].loads[k].fases =
              static_cast<FASES>(atoi(getField(dados, 3)));
  ...
  int leituraDSHNT(DBAR *barras, int numeroBarras) {
    ...
    char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar
                    o ponteiro da string lida do arquivo de forma a realizar o
                    loop no sscanf. */
    int i, aux, k; /* Variáveis contadores para percorrer o arquivo e a string
                      de leitura. */
    dados = new char[100];
    // Le o arquivo de curva de cargas até o fim
    while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
      ...
      if (aux != -1) {
        ...
        // if (((barras)[aux].shunts = (DSHNT *)realloc((barras)[aux].shunts,
        // (barras)[aux].nshunts  * sizeof(DSHNT)))==NULL)
        // {
        //     printf("Erro -- Nao foi possivel alocar espaco de memoria para
        //     shunts !!!!"); exit(1);
        // }
        k = (barras)[aux].nshunts - 1;

        (barras)[aux].shunts[k].ID = (barras)[aux].ID;
        (barras)[aux].shunts[k].lig =
            static_cast<LIGACAO>(atoi(getField(dados, 2)));
        (barras)[aux].shunts[k].fases =
            static_cast<FASES>(atoi(getField(dados, 3)));
  ...
  int leituraDGD(DBAR *barras, int numeroBarras) {
    ...
    while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
      ...
      if (aux != -1) {
        ...
        // if (((barras)[aux].gds = (DGD *)realloc((barras)[aux].gds,
        // (barras)[aux].ngds * sizeof(DGD)))==NULL)
        // {
        //     printf("Erro -- Nao foi possivel alocar espaco de memoria para
        //     gds !!!!"); exit(1);
        // }
        ...
        (barras)[aux].gds[k].lig =
            static_cast<LIGACAO>(atoi(getField(dados, 2)));
        (barras)[aux].gds[k].fases =
            static_cast<FASES>(atoi(getField(dados, 3)));
  ...
  DRAM *leituraDLIN(int &numeroRamos, DBAR *barras, int numeroBarras) {
    ...
    char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar
                    o ponteiro da string lida do arquivo de forma a realizar o
                    loop no sscanf. */
    long int contador = 0, i; /* Variáveis contadores para percorrer o arquivo e
                                 a string de leitura. */
    int carac,
        numLinhas =
            0; /* Variável com o número de linhas do arquivo a serem lidas. */
    ...
    // {
    //     printf("Erro -- Nao foi possivel alocar espaco de memoria para as
    //     linhas !!!!"); exit(1);
    // }
    ...
    while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
      ...
      (m_ramos)[contador].linha.Baa =
          comp * atof(getField(dados, 17)) / 1000000;
      (m_ramos)[contador].linha.Bab =
          comp * atof(getField(dados, 18)) / 1000000;
      (m_ramos)[contador].linha.Bac =
          comp * atof(getField(dados, 19)) / 1000000;
      (m_ramos)[contador].linha.Bbb =
          comp * atof(getField(dados, 20)) / 1000000;
      (m_ramos)[contador].linha.Bbc =
          comp * atof(getField(dados, 21)) / 1000000;
      (m_ramos)[contador].linha.Bcc =
          comp * atof(getField(dados, 22)) / 1000000;
  ...
  DRAM *leituraDTRF(int &numeroTrafos, DBAR *barras, int numeroBarras) {
    ...
    char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar
                    o ponteiro da string lida do arquivo de forma a realizar o
                    loop no sscanf. */
    int contador = 0, i; /* Variáveis contadores para percorrer o arquivo e a
                            string de leitura. */
    int carac,
        numLinhas =
            0; /* Variável com o número de linhas do arquivo a serem lidas. */
    ...
    // DRAM *m_ramos;
    DRAM *m_ramos = new DRAM[numLinhas];
    // if (((ramos) = (DRAM *)realloc((ramos), (numeroRamos + numLinhas) *
    // sizeof(DRAM)))==NULL)
    // {
    //     printf("Erro -- Nao foi possivel alocar espaco de memoria para as
    //     linhas !!!!"); exit(1);
    ...
    while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
      ...
      (m_ramos)[contador].trafo.R =
          atof(getField(dados, 7)) * pow((m_ramos)[contador].trafo.Vsec, 2) /
          ((m_ramos)[contador].trafo.Snominal * 1000) /
          3; // R*(Vpri^2/(S*1000))
      (m_ramos)[contador].trafo.X =
          atof(getField(dados, 8)) * pow((m_ramos)[contador].trafo.Vsec, 2) /
          ((m_ramos)[contador].trafo.Snominal * 1000) / 3;
      (m_ramos)[contador].trafo.lig_pri =
          static_cast<LIGACAO>(atoi(getField(dados, 9)));
      (m_ramos)[contador].trafo.lig_sec =
          static_cast<LIGACAO>(atoi(getField(dados, 10)));
  ...
  DRAM *leituraDREG(int &numeroRegs, DBAR *barras, int numeroBarras) {
    ...
    // sizeof(DRAM)))==NULL)
    // {
    //     printf("Erro -- Nao foi possivel alocar espaco de memoria para os
    //     reguladores de tensão !!!!"); exit(1);
    // }
    contador = 0;
    // Le o arquivo de curva de cargas até o fim
    while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
      ...
      (m_ramos)[contador].regulador.fases =
          static_cast<FASES>(atoi(getField(dados, 3)));
      ...
      (m_ramos)[contador].regulador.R =
          atof(getField(dados, 8)) *
          pow((m_ramos)[contador].regulador.Vnom, 2) /
          ((m_ramos)[contador].regulador.Snominal * 1000) / 3;
      (m_ramos)[contador].regulador.X =
          atof(getField(dados, 9)) *
          pow((m_ramos)[contador].regulador.Vnom, 2) /
          ((m_ramos)[contador].regulador.Snominal * 1000) / 3;
      (m_ramos)[contador].regulador.lig =
          static_cast<LIGACAO>(atoi(getField(dados, 10)));
  ...
  DRAM *leituraDSWTC(int &numeroChaves, DBAR *barras, int numeroBarras) {
    ...
    rewind(arquivo);
    DRAM *m_ramos = new DRAM[numLinhas];
    // if (((*ramos) = (DRAM *)realloc((*ramos), (numeroRamos[0] + numLinhas) *
    // sizeof(DRAM)))==NULL)
    // {
    //     printf("Erro -- Nao foi possivel alocar espaco de memoria para as
    //     chaves !!!!"); exit(1);
    ...
    while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
      ...
      (m_ramos)[contador].fases = static_cast<FASES>(atoi(getField(dados, 3)));
      (m_ramos)[contador].estado =
          static_cast<ESTADO>(atoi(getField(dados, 4)));
  ...
  DRAM *joinDRAM(DRAM *ramos, DRAM *trafos, DRAM *reguladores, DRAM *chaves) {
    DRAM *m_ramos =
        new DRAM[numeroRamos + numeroTrafos + numeroRegs + numeroChaves];
    ...
    if (reguladores != NULL) {
      for (i = 0; i < numeroRegs; i++) {
        ...
        m_ramos[i + numeroRamos + numeroTrafos].regulador =
            reguladores[i].regulador;
        ...
        for (j = 0; j < 3; j++) {
          m_ramos[i + numeroRamos + numeroTrafos].tap_pri[j] =
              reguladores[i].tap_pri[j];
          m_ramos[i + numeroRamos + numeroTrafos].tap_sec[j] =
              reguladores[i].tap_sec[j];
        }
      }
    }
    ...
    if (chaves != NULL) {
      for (i = 0; i < numeroChaves; i++) {
        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].DE =
            chaves[i].DE;
        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].PARA =
            chaves[i].PARA;
        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].k = chaves[i].k;
        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].m = chaves[i].m;
        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].fases =
            chaves[i].fases;
        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].tipo =
            chaves[i].tipo;
        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].estado =
            chaves[i].estado;

        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].linha =
            chaves[i].linha;
        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].trafo =
            chaves[i].trafo;
        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].regulador =
            chaves[i].regulador;

        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].Ypp =
            chaves[i].Ypp;
        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].Yps =
            chaves[i].Yps;
        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].Ysp =
            chaves[i].Ysp;
        m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].Yss =
            chaves[i].Yss;

        for (j = 0; j < 3; j++) {
          m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].tap_pri[j] =
              chaves[i].tap_pri[j];
          m_ramos[i + numeroRamos + numeroTrafos + numeroRegs].tap_sec[j] =
              chaves[i].tap_sec[j];
        }
      }
    }
  ...
  int leituraVinicial(DBAR *barras, int numeroBarras) {
    ...
    char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar
                    o ponteiro da string lida do arquivo de forma a realizar o
                    loop no sscanf. */
    int i, aux;  /* Variáveis contadores para percorrer o arquivo e a string de
                    leitura. */
    ...
    while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
      ...
      if (aux != -1) {
        ...
        (barras)[aux].Vinicial[0] =
            std::complex<double>(Va * cos(Ta), Va * sin(Ta));
        (barras)[aux].Vinicial[1] =
            std::complex<double>(Vb * cos(Tb), Vb * sin(Tb));
        (barras)[aux].Vinicial[2] =
            std::complex<double>(Vc * cos(Tc), Vc * sin(Tc));
      }
    }
    return (0);
  }
  GRAFO *geraGrafo(DBAR *barras, int numeroBarras, DRAM *allRamos,
                   int numeroRamos) {
    long int i, j, k;
    ...
    for (i = 0; i < numeroRamos; i++) {
      barraDe = allRamos[i].k;
      barraPara = allRamos[i].m;
      nadj = (m_grafo)[barraDe].numeroAdjacentes;

      (m_grafo)[barraDe].adjacentes[nadj].ramo = &allRamos[i];
      (m_grafo)[barraDe].adjacentes[nadj].idNo = barraPara;
      (m_grafo)[barraDe].adjacentes[nadj].tipo = allRamos[i].tipo;
      (m_grafo)[barraDe].adjacentes[nadj].estado = allRamos[i].estado;
      (m_grafo)[barraDe].adjacentes[nadj].nmed = 0;
      (m_grafo)[barraDe].adjacentes[nadj].idram = i;
      (m_grafo)[barraDe].numeroAdjacentes++;
      if (ramos[i].tipo == 1) {
        (m_grafo)[barraDe].adjacentes[nadj].relacao =
            allRamos[i].trafo.Vsec / allRamos[i].trafo.Vpri;
      }
      (m_grafo)[barraDe].adjacentes[nadj].medidores =
          (DMED **)malloc(30 * sizeof(DMED *));

      nadj = (m_grafo)[barraPara].numeroAdjacentes;
      (m_grafo)[barraPara].adjacentes[nadj].ramo = &allRamos[i];
      (m_grafo)[barraPara].adjacentes[nadj].idNo = barraDe;
      (m_grafo)[barraPara].adjacentes[nadj].tipo = allRamos[i].tipo;
      (m_grafo)[barraPara].adjacentes[nadj].estado = allRamos[i].estado;
      (m_grafo)[barraPara].adjacentes[nadj].nmed = 0;
      (m_grafo)[barraPara].adjacentes[nadj].idram = i;
      (m_grafo)[barraPara].numeroAdjacentes++;
      if (allRamos[i].tipo == 1) {
        (m_grafo)[barraPara].adjacentes[nadj].relacao =
            allRamos[i].trafo.Vpri / allRamos[i].trafo.Vsec;
      }
      (m_grafo)[barraPara].adjacentes[nadj].medidores =
          (DMED **)malloc(30 * sizeof(DMED *));
    }
    return (m_grafo);
  }
  ...
  DMED *leituraMedidas(DRAM *ramos, int numeroRamos, DBAR *barras,
    ...
    char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar
                    o ponteiro da string lida do arquivo de forma a realizar o
                    loop no sscanf. */
    long int contador = 0, i, j; /* Variáveis contadores para percorrer o
                                    arquivo e a string de leitura. */
    int carac,
        numLinhas =
            0; /* Variável com o número de linhas do arquivo a serem lidas. */
    ...
    while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
      ...
      (m_medidas)[contador].ligado =
          static_cast<ESTADO>(atoi(getField(dados, 1)));
      ...
      (m_medidas)[contador].fases =
          static_cast<FASES>(atoi(getField(dados, 5)));
      ...
      numeroMedidas[(m_medidas)[contador].tipo]
                   [(m_medidas)[contador].fases - 1]++;
      ...
      case 13: // PMU de corrente retangular imaginário
        for (i = 0; i < numeroRamos; i++) {
          if (((m_medidas)[contador].DE == ramos[i].DE) &&
              ((m_medidas)[contador].PARA == ramos[i].PARA)) {
            (m_medidas)[contador].k = ramos[i].k;
            (m_medidas)[contador].m = ramos[i].m;
            (m_medidas)[contador].ramo = i;
          }
          if (((m_medidas)[contador].DE == ramos[i].PARA) &&
              ((m_medidas)[contador].PARA == ramos[i].DE)) {
  ...
  DMED *processaMedidas(DMED *medidas, int numeroMedidas, GRAFO *grafo,
                        int numeroBarras) {
    int i, k, m, l;
    int ind, adj;
    double regua;
    ...
    for (i = 0; i < numeroMedidas; i++) {
        m_medidas[i].DE = medidas[i].DE;
        m_medidas[i].PARA = medidas[i].PARA;
        m_medidas[i].id = medidas[i].id;
        m_medidas[i].k = medidas[i].k;
        m_medidas[i].m = medidas[i].m;
        m_medidas[i].ramo = medidas[i].ramo;
        m_medidas[i].fases = medidas[i].fases;
        m_medidas[i].tipo = medidas[i].tipo;
        m_medidas[i].idAlim = medidas[i].idAlim;
        m_medidas[i].idArea = medidas[i].idArea;
        m_medidas[i].idFront = medidas[i].idFront;
        m_medidas[i].ligado = medidas[i].ligado;
        m_medidas[i].zmed = medidas[i].zmed;
        m_medidas[i].sigma = medidas[i].sigma;
        m_medidas[i].prec = medidas[i].prec;
        m_medidas[i].par = medidas[i].par;
        m_medidas[i].h = medidas[i].h;
        m_medidas[i].nvar = medidas[i].nvar;
        m_medidas[i].reguaH = medidas[i].reguaH;
        m_medidas[i].H = medidas[i].H;
    ...
    for (i = 0; i < numeroMedidas; i++) {
      ...
      case 0: // Medida de Fluxo de Potência Ativa em kW
        ...
        (m_medidas)[i].nvar = 12;
        (m_medidas)[i].reguaH =
            (double *)malloc((m_medidas)[i].nvar * sizeof(double));
        (m_medidas)[i].H =
            (double *)malloc((m_medidas)[i].nvar * sizeof(double));
      ...
      case 4: // Medida de Magnitude de Tensão - kV
        ...
        case 5:
        case 6:
          Vbase = grafo[k].Vbase * (pow(3.0, 0.5));
          break;
        }
      ...
      case 6: // Medida de Magnitude de Corrente em A - Fluxo
        Vbase = grafo[k].Vbase;
        (m_medidas)[i].zmed = (m_medidas)[i].zmed /
                              ((Sbase / 1000) / (pow(3.0, 0.5) * (Vbase / 1000)));
        // (m_medidas)[i].sigma =
        //     (m_medidas)[i].sigma /
        //     ((Sbase / 1000) / (pow(3, 0.5) * (Vbase / 1000)));
      ...
      case 10: // Medida PMU de injeção de corrente retangular real
        Vbase = grafo[k].Vbase;
        (m_medidas)[i].zmed =
            (m_medidas)[i].zmed / ((Sbase / 1000) / ((Vbase / 1000)));
        // (m_medidas)[i].sigma =
        //     (m_medidas)[i].sigma / ((Sbase / 1000) / ((Vbase / 1000)));
    ...
    // Associa os medidores ao grafo e transforma medidas em pu (equivalente de
    // corrente para o AMB e Baran precisa do par de medida)
    for (i = 0; i < numeroMedidas; i++) {
      k = (m_medidas)[i].k;
      m = (m_medidas)[i].m;
      ...
      case 0: // Medida de Fluxo de Potência Ativa em kW
        for (j = 0; j < numeroMedidas; j++) {
          if (((m_medidas)[j].tipo == 1) && ((m_medidas)[j].k == k) &&
              ((m_medidas)[j].m == m)) {
            (m_medidas)[i].par = j;
            (m_medidas)[j].par = i;
            j = numeroMedidas;
      ...
      case 2: // Medida de Injeção de Potência Ativa em kW
        for (j = 0; j < numeroMedidas; j++) {
          if (((m_medidas)[j].tipo == 3) && ((m_medidas)[j].k == k) &&
              ((m_medidas)[j].m == m)) {
            (m_medidas)[i].par = j;
            (m_medidas)[j].par = i;
            j = numeroMedidas;
      ...
      case 4: // Medida de Magnitude de Tensão - kV
        for (j = 0; j < numeroMedidas; j++) {
          if (((m_medidas)[j].tipo == 5) && ((m_medidas)[j].k == k) &&
              ((m_medidas)[j].m == m)) {
            (m_medidas)[i].par = j;
            (m_medidas)[j].par = i;
            j = numeroMedidas;
      ...
      case 6: // Medida de Magnitude de Corrente em A
        for (j = 0; j < numeroMedidas; i++) {
          if (((m_medidas)[j].tipo == 7) && ((m_medidas)[j].k == k) &&
              ((m_medidas)[j].m == m)) {
            (m_medidas)[i].par = j;
            (m_medidas)[j].par = i;
            j = numeroMedidas;
